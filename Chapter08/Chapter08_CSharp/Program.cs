    ...
    static void MainDecisionTrees()
    {
      ...
      var tree =
          new DecisionQuery
          {
              ...
              Test = (client) => client.Income > 40000,
              // Sub-trees can be 'DecisionResult' or 'DecisionQuery'
              Positive = new DecisionQuery() { Title = "Has criminal record" , 
                                               Test = (client) => client.CriminalRecord,
                                               Positive = new DecisionResult { Result = false},
                                               Negative = new DecisionResult { Result = true }},
              Negative = new DecisionQuery() { Title = "Years in job", 
                                               Test = (client) => client.YearsInJob > 1,
                                               Positive = new DecisionResult { Result = true},
                                               Negative = new DecisionQuery() { Title = "Uses credit card", 
                                                                                Test = cl => cl.UsesCreditCard,
                                                                                Positive = new DecisionResult { Result = true},
                                                                                Negative = new DecisionResult { Result = false }
